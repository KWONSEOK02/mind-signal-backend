...
- 정규식 기반 커스텀 규칙으로 BE 특화 경계 정의 가능함
  (예: `no-mongoose-in-features`, `no-upward-from-shared`)
...
BE `.dependency-cruiser.cjs` 핵심 규칙 4종 (Wave 2에서 구현):

```js
// (a) no-mongoose-in-features: 05-features → mongoose/mongodb 직접 import 금지
// (b) no-mongoose-direct-in-processes: 02-processes → mongoose/mongodb 직접 import 금지
//     (06-entities 경유 접근은 허용 — orchestration 레이어 특성)
// (c) no-upward-from-shared: 07-shared → 상위 레이어 import 금지
// (d) no-fs-in-features: 05-features → fs/path 직접 접근 금지 (07-shared/lib 경유)
```
