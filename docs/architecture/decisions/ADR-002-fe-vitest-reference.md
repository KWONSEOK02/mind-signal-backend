# ADR-002: FE 테스트 러너로 Vitest를 유지함 (FE 전용 결정)

<!-- ⚠️ 이 ADR은 FE(mind-signal-frontend) 전용 결정의 정보성 참조 사본임.
     BE 작업 시 영향 없음. BE는 여전히 Jest + supertest를 사용함.
     BE 에이전트가 이 파일을 읽고 Vitest 관련 도구를 도입하는 일이 없도록
     상단에 명시함. -->

- **Status**: Accepted
- **Date**: 2026-04-18
- **Applies to**: FE only (informational reference for BE agents)
- **Deciders**: @gs07103
...
- 이 ADR 파일은 "FE 결정을 BE 에이전트가 오인하지 않도록" 정보성 참조 사본으로만 존재함
