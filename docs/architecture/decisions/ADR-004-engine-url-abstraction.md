...
## 2PC 확장 경계 제약 (HARD CONSTRAINTS)

아래 제약은 `.claude/rules/architecture.md`에도 박제됨.
**이 제약을 위반하는 코드는 Phase 15 이후 모든 PR에서 차단 대상임.**

| 제약 | 내용 |
|---|---|
| **Engine-location abstraction** | 서비스 코드에서 `child_process.spawn` 직접 호출 금지. 반드시 `engineRegistryService` 경유 |
